...
echo "$($PSQL "SELECT DISTINCT(teams.name) FROM games LEFT JOIN teams on games.winner_id = teams.team_id OR games.opponent_id = teams.team_id WHERE games.year = 2014 AND games.round = 'Eighth-Final' ORDER BY teams.name;")"
